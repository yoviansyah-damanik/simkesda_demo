    ...
    <table class="bordered">
        ...
        <tbody>
            ...
            @for ($z = 1; $z <= count($label); $z++)
                <tr>
                    <th colspan={{ 3 + $data->count() }}
                        style="text-align:center; padding:0; background-color: #267a5d40; color:#000;">
                        {{ $numb[$z - 1] . '. ' . $label[$z - 1] }}
                    </th>
                </tr>
                @for ($x = 0; $x < count(${'atts_' . $z}); $x++)
                    ...
                    <tr>
                        ...
                        @endfor --}}
                        @foreach ($data as $val)
                            <td style="text-align:right;">@numb($val->{${'atts_' . $z}[$x]['attribute']})</td>
                            @php
                                $total += $val->{${'atts_' . $z}[$x]['attribute']};
                            @endphp
                        @endforeach
