<div class="content">
    <div class="filter-side filter-side-3">
        <form action="" method="get">
            <div class="input-group">
                <select name="bulan" id="bulan" class="form-select" required>
                    <option value="semua_data" @if (!$tahun) selected @endif>Semua bulan
                    </option>
                    @for ($x = 1; $x <= 12; $x++)
                        <option value="{{ $x }}" @if ($bulan == $x) selected @endif>
                ...
                @if ($data->count() > 0)
                    @foreach ($data as $val)
                        <tr>
                            ...
                            <td>{{ Carbon\Carbon::parse(date('Y-' . $val->bulan . '-01'))->translatedFormat('F') }}
                            </td>
                            <td>{{ $val->tahun }}</td>
                            ...
                            <td>
                                <a class="btn btn-sm btn-info text-white"
                                    href="{{ route('dashboard.priority.monthly.show', $val->slug) }}"
                                    data-bs-tooltip="tooltip" data-bs-placement="bottom" title="Lihat!">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a class="btn btn-sm btn-warning text-white"
                                    href="{{ route('dashboard.priority.monthly.edit', $val->slug) }}"
                                ...
                                <button class="btn btn-sm btn-danger hapus-data" data-type="data_bulanan"
                                    data-value="{{ $val->slug }}" data-month="{{ $val->bulan }}"
                                    data-year="{{ $val->tahun }}" data-bs-tooltip="tooltip"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>
                    Apakah anda yakin akan menghapus data bulanan bulan <span class="bulan"></span> tahun <span
                        class="tahun"></span>?
                </p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
                <form action="{{ route('dashboard.priority.monthly.destroy') }}" method="post">
                    @csrf
                    @method('delete')
