...
@section('container')
    <div class="container">
            ...
            <div class="d-lg-flex justify-content-between align-items-start">
                <div class="d-block">
                    <div class="d-lg-flex justify-content-start">
                        <div class="me-lg-2 me-0 col-lg-7">
                            <div class="form-group">
                                <label for="sumber_data" class="form-label form-label-bold">
                                    Sumber Data
                                    <p class="small helper">
                                        Sumber Template Prioritas - Data Sasaran diperoleh.
                                    </p>
                                </label>
                    ...
                    @role('Superadmin|Administrator')
                        @if ($data->status_verifikasi == 2 || $data->status_verifikasi == 3)
                            <div class="btn-group mt-1 mt-sm-0">
                                <form action="{{ route('dashboard.priority.target.approval', $data->slug) }}" method="post">
                                    @csrf
                                    @method('put')
                            ...
                            @if ($data->status_verifikasi == 0 || $data->status_verifikasi == 3)
                                <div class="btn-group mt-1 mt-sm-0">
                                    <a class="btn-custom btn-edit"
                                        href="{{ route('dashboard.priority.target.edit', $data->slug) }}">
                                        <i class="fas fa-pen"></i>
                                        Edit
                                    </a>
                                    <form action="{{ route('dashboard.priority.target.submission', $data->slug) }}" method="post">
                                        @csrf
                                        @method('put')
        ...
        @include('backend.partials.field_data.prioritas_data_sasaran')
    </div>
@endsection
