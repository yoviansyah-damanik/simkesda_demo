...
@section('container')
    <div class="container">
                    ...
                    @role('Superadmin|Administrator')
                        @if ($data->status_verifikasi == 2 || $data->status_verifikasi == 3)
                            <div class="btn-group mt-1 mt-sm-0">
                                <form action="{{ route('dashboard.spm.target.approval', $data->slug) }}" method="post">
                                    @csrf
                                    @method('put')
                    ...
                    @else
                        @role('Puskesmas')
                            @if ($data->status_verifikasi == 0 || $data->status_verifikasi == 3)
                                <div class="btn-group mt-1 mt-sm-0">
                                    <a class="btn-custom btn-edit" href="{{ route('dashboard.spm.target.edit', $data->slug) }}">
                                        <i class="fas fa-pen"></i>
                                        Edit
                                    </a>
                                    <form action="{{ route('dashboard.spm.target.submission', $data->slug) }}" method="post">
                                        @csrf
                                        @method('put')
        ...
        @include('backend.partials.field_data.spm_data_sasaran')
    </div>
@endsection
