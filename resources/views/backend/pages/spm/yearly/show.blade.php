...
@section('container')
    <div class="container">
            ...
            <div class="d-lg-flex justify-content-between align-items-start">
                <div class="d-block">
                    <div class="d-lg-flex justify-content-start">
                        <div class="me-lg-2 me-0 col-md-7">
                    ...
                    @role('Superadmin|Administrator')
                        @if ($data->status_verifikasi == 2 || $data->status_verifikasi == 3)
                            <div class="btn-group mt-1 mt-sm-0">
                                <form action="{{ route('dashboard.spm.yearly.approval', $data->slug) }}" method="post">
                                    @csrf
                                    @method('put')
                    ...
                    @else
                        @role('Puskesmas')
                            @if ($data->status_verifikasi == 0 || $data->status_verifikasi == 3)
                                <div class="btn-group mt-1 mt-sm-0">
                                    <a class="btn-custom btn-edit" href="{{ route('dashboard.spm.yearly.edit', $data->slug) }}">
                                        <i class="fas fa-pen"></i>
                                        Edit
                                    </a>
                                    <form action="{{ route('dashboard.spm.yearly.submission', $data->slug) }}" method="post">
                                        @csrf
                                        @method('put')
        ...
        @include('backend.partials.field_data.spm_data_tahunan')
    </div>
@endsection
